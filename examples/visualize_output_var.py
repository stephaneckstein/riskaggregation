        ...
        for m_ind, M_T in enumerate(M_TYPE):
            for ul_ind, UL in enumerate(UB_OR_LB):
                for uol in range(LB_UB_RANGE):
                    ...
                    try:
                        val = np.load(save_path + '_VaR_value.npy')
                        print(val)
                        opti_x = np.load(save_path + '_VaR_optx.npy')
                        opti_w = np.load(save_path + '_VaR_optw.npy')
                    except:
                        val = 0
                    ...
                    if PLOT_OPTIMIZERS:
                        ...
                        if C_T == 1:
                            plt.title('Value at Risk optimizer, unconditional constraint')
                        elif C_T == 3:
                            plt.title('Value at Risk optimizer, conditional constraint')
                        else:
                            plt.title('Value at Risk optimizer, $x={bound:.3f}$'.format(bound=cur_bound))
...
for d_ind, D in enumerate(D_LIST):
    for c_ind, C_T in enumerate(C_TYPE):
        for m_ind, M_T in enumerate(M_TYPE):
            ...
            if C_T == 1:
                plt.title(r'Value at risk, $d={}$'.format(D))
            if C_T == 3:
                plt.title(r'Value at risk, $d={}$'.format(D))
            plt.xlabel('Level of bound')
            plt.ylabel('Worst-case VaR')
            plt.savefig('../data/figures/var_'+str(round(100*ALPHA))+'_'+str(d_ind)+'_'+str(c_ind)+'_'+str(m_ind)
